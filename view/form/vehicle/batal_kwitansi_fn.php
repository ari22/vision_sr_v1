<script>
        ...
        $('#group_by').combobox({
            onSelect: function (index, row) {
                ...
                $('#form_validation #id').val('');
                 ...
                 $('#form_validation #canc_note,#chassis,#kwit_no,#so_no,#sal_inv_no,#veh_code,#veh_name,#chassis,#veh_type,#engine,#veh_model,#veh_brand,#veh_transm,#veh_year,#color_code,#color_name,#color_type,#cust_code,#cust_name,#cust_addr,#cust_area,#cust_city,#cust_zipcode').val('');
            ...
            onSelect: function (index, rowData) {
                ...
                if (rowData !== '[]') {
                    ...
                    $.each(rowData, function (i, v) {
                        ...
                        if (i == 'so_date' || i == 'kwit_date' || i == 'sal_date') {
                            if (v !== '0000-00-00') {
    ...
    function cancelling() {
        var kwit_no = $("#kwit_no").val();
        $.messager.confirm('Confirmation', 'This process will cancel Receipt No. <b>' + kwit_no + '</b>. Are you sure you want to proceed? ', function (r) {
            var c_note = $("#canc_note").val();
            var id = $("#id").val();

            if (c_note !== '') {
                $.post(site_url + 'form/UJKwitcancel', {canc_by: '<?php echo $_SESSION['C_USER']; ?>', canc_note: c_note, id: id,form:'kwitansi'}, function (json) { 
