<?php
	...
	$optRepGroup = array
	(
		array("Sales Rep","by_srep_monthly"),
...
?>
		...
		$(function(){
		  ...
		  $("#closeTab").click(function() {
					$.post("clear.php",function(data){
					 window.parent.$('#tabRptSPKBySrep').tabs('close','Create List'); 
					 location.reload();     
			  });
		  });
		});
	...
	<div data-options="region:'center'" title="" >
		<div id="tabRptSPKBySrep" class="easyui-tabs" data-options="fit:true,border:false,plain:true">
...
function doSearch(){
			url ="&lookup=mst/tpl_lookup&table=veh_spk&pk=so_no&sk=cust_name";
			...
			//url = url + "&fields=srep_name,so_no,so_date,cust_code,cust_name,veh_code,veh_name,chassis,engine,color_code,color_name";
			filter = searchFilter();
			if (filter.length>>0)
			{
		...
		addTab($('#group_by').combobox('getText'),'loader.php?gen_type=view&form_type=rpt&form_name=advance/jqw_spk'+url);
	}
