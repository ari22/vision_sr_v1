...
<div data-options="region:'east',split:true,title:'Filter',split:false" style="width:465px;padding:10px;" id='eastpanel'>
		...
		<table>
				...
				<table cellpadding="5" style="margin: 0cm 0px 0px 0px">
					<?php			
						localcombobox('rep_type','Cetak Faktur PO',200,$optRepType);
						datebox('date_from','Tgl PO',200);
	...
	<div data-options="region:'center'" title="PO Kendaraan" >
	<table id="dtrpt" class="easyui-datagrid"  
			...
			<thead>
				<tr>
					...
					<th data-options="field:'po_date',width:80,halign:'center',sortable:true">Tanggal PO</th>
					<th data-options="field:'po_no',width:100,halign:'center',sortable:true">Nomor PO</th>
					...
					<th data-options="field:'po_made_by',width:100,halign:'center',sortable:true">Dibuat oleh</th>
					<th data-options="field:'po_appr_by',width:100,halign:'center',sortable:true">Disetujui oleh</th>
					<th data-options="field:'qty',width:100,halign:'center',sortable:true">Qty</th>
					<th data-options="field:'unit_price',width:100,halign:'center',sortable:true">Harga Beli</th>
				</tr>
			</thead>
		</table>
...
function doSearch(){
			//alert('yes');
			url ="services/runCRUD.php?func=read&lookup=mst/tpl_lookup&table=veh_po&pk=po_no&sk=supp_name";
			url = url + "&date_field=po_date&date_from="+$('#date_from').datebox('getValue') + "&date_to="+$('#date_to').datebox('getValue');
			url = url + "&optRep=" + $('#rep_type').combogrid('getValue');
			url = url + "&fields=po_date,po_no,veh_brand,veh_type,veh_year,veh_transm,color_name,po_made_by,po_appr_by,qty,unit_price";
			filter = searchFilter();
			if (filter.length>>0)
			{
