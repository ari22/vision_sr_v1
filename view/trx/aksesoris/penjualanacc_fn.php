<script>
    ...
    var save_url = site_url + 'transaction/accessories/saveAccessories/<?php echo $_SESSION['C_USER']; ?>/VSL';
    var unclose_url = site_url + 'transaction/accessories/unclosePenjualan/<?php echo $_SESSION['C_USER']; ?>';
    var close_url = site_url + 'transaction/accessories/closePenjualan/<?php echo $_SESSION['C_USER']; ?>';
    ...
    $("#sal_inv_no").combogrid({
        method: 'get',
        url: site_url + '<?php echo 'builder/combogrid/' . encrypt_decrypt('encrypt', 'veh_slh'); ?>/accessories/?grid=true',
        idField: 'sal_inv_no',
        ...
        loadMsg: 'Please wait...',
        columns: [[
                {field: 'sal_inv_no', title: '<?php getCaption('No. Faktur'); ?>', width: 120},
                {field: 'cust_code', title: '<?php getCaption('Kode Pelanggan'); ?>', width: 120},
                {field: 'cust_name', title: '<?php getCaption('Nama Pelanggan'); ?>', width: 200},
                {field: 'srep_code', title: '<?php getCaption('Kode Sales'); ?>', width: 120},
                {field: 'srep_name', title: '<?php getCaption('Nama Sales'); ?>', width: 200}
            ]]
    });

    $('#form_validation #sal_inv_no').combogrid({
        onSelect: function (index, row) {
            $('#form_validation #cust_code').val(row.cust_code);
            $('#form_validation #cust_name').val(row.cust_name);
            $('#form_validation #srep_code').val(row.srep_code);
            $('#form_validation #srep_name').val(row.srep_name);

            if (row.opn_date !== '0000-00-00') {
                var opn_date = dateSplit(row.opn_date);
                $('#form_validation #opn_date').datebox('setValue', opn_date);
            }

            $('#form_validation #wrhs_code').combogrid('setValue', row.wrhs_code);
        }
    });
    ...
    function read_show(nav) {
        ...
        $.post(site_url + 'transaction/accessories/read_slh/VSL', {table: table, nav: nav, id: id}, function (json) { //alert(json)
    ...
    function print_sc(key) {
        ...
        if (key == 'print') {
            ...
            var url = site_url + 'transaction/accessories/outputpdf/ <?php echo encrypt_decrypt('encrypt', 'acc_slh'); ?>/' + ids + '/<?php echo $_SESSION['C_USER']; ?>/print/<?php echo encrypt_decrypt('encrypt', 'sal_inv_no'); ?>/<?php echo encrypt_decrypt('encrypt', 'ASC'); ?>';
        ...
        if (key == 'screen') {
            var ids = $("#form_validation #id").val();
            var url = site_url + 'transaction/accessories/outputpdf/<?php echo encrypt_decrypt('encrypt', 'acc_slh'); ?>/' + ids + '/<?php echo $_SESSION['C_USER']; ?>/screen/<?php echo encrypt_decrypt('encrypt', 'sal_inv_no'); ?>/<?php echo encrypt_decrypt('encrypt', 'ASC'); ?>';
            window.open(url);
        }

        if (key == 'download') {
            var ids = $("#form_validation #id").val();
            var url = site_url + 'transaction/accessories/outputpdf/<?php echo encrypt_decrypt('encrypt', 'acc_slh'); ?>/' + ids + '/<?php echo $_SESSION['C_USER']; ?>/download/<?php echo encrypt_decrypt('encrypt', 'sal_inv_no'); ?>/<?php echo encrypt_decrypt('encrypt', 'ASC'); ?>';
    ...
    function condReady() {
        ...
        var sal_inv_no = $("#form_validation #sal_inv_no").combogrid('getValue');
        ...
        $("#form_validation #sal_inv_no,#inv_type,#cust_code,#cust_name,#srep_code,#srep_name,#tot_item, #tot_qty,#tot_price,#inv_disc,#inv_bt,#inv_vat,#inv_at,#inv_stamp,#inv_total").attr('disabled', true);
    ...
    function condAdd() {
        ...
        /*
         $.post(site_url + 'transaction/accessories/get_number/VSL', function (num) {
                ...
                form.form('submit', {
                    ...
                    success: function (data) {
                        ...
                        } else
                        {
                            $('.loader').show();
                            read_show('');
                            showAlert("Error while Unclosing", '<font color="red">' + obj.message + '</font>');
                        }
                        scrlTop();
                    }
                });
    ...
    function saveDiskon() {
        ...
        $("#form_diskon").form('submit', {
            ...
            success: function (data) {
                ...
                {
                    $('#SetDiskonWindow').window('close');
                    showAlert("Saving failed", '<font color="red">' + obj.message + '</font>');
                }
            }
        });
    }
