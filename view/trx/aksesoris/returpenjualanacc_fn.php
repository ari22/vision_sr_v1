<script>
    ...
    var unclose_url = site_url + 'transaction/accessories/uncloseReturnPenj/<?php echo $_SESSION['C_USER']; ?>';
    var close_url = site_url + 'transaction/accessories/closeReturnPenj/<?php echo $_SESSION['C_USER']; ?>';
    ...
    $("#sal_inv_no").combogrid({
        onSelect: function (index, rowData) {
             var vdate = dateSplit(rowData.sal_date);
             $("#form_validation #sal_date").datebox('setValue', vdate);
        }
    });
    function tableId() {
        ...
        $.post(site_url + 'crud/read', {table: table, nav: nav, id: id}, function (json) { //alert(json)
            ...
            if (json !== '[]') {
                ...
                $.each(rowData, function (i, v) {
                    ...
                    if (i == 'cust_name' || i == 'wrhs_code' || i == 'srep_name' || i == 'sal_inv_no') {
                        $("#form_validation #" + i).combogrid('setValue', v);
                    }

                    if (i == 'opn_date' || i == 'rsl_date' || i == 'sal_date' || i == 'due_date' || i == 'cls_date' || i == 'so_date') {
                        if (v !== '0000-00-00') {
                ...
                var rsl_inv_no = rowData.rsl_inv_no;
    ...
    function doSearch() {
        $("#tableSearch").datagrid('load', {
            field1: $('#rsl_inv_no2').val(),
            field2: $('#rsl_date2').datebox('getValue')
        });
    }
    ...
    function print_sc(key) {
         ...
         var url = site_url + 'transaction/accessories/outputpdf/<?php echo encrypt_decrypt('encrypt', 'acc_rslh'); ?>/' + ids + '/<?php echo $_SESSION['C_USER']; ?>/'+key+'/<?php echo encrypt_decrypt('encrypt', 'rsl_inv_no'); ?>/<?php echo encrypt_decrypt('encrypt', 'ARC'); ?>#toolbar=0';
    ...
    function table_grid(ttable, rsl_inv_no) {
        ttable.datagrid({
            method: 'post',
            url: site_url + '<?php echo 'builder/datagrid/' . encrypt_decrypt('encrypt', 'acc_rsld'); ?>/rsl_inv_no/' + rsl_inv_no + '/?grid=true',
            idField: 'id',
            fitColumns: false,
            singleSelect: true,
            nowrap: true,
            ...
            columns: [[
                    ...
                    {field: 'part_name', title: '<?php getCaption("Nama Barang"); ?>', width: 220, height: 20, sortable: true},
    ...
    function condReady() {
        ...
        $('#form_validation #rsl_date, #cls_date').datebox('disable');

        $("#form_validation #rsl_inv_no,#inv_type,#cust_code,#srep_code,#tot_item, #tot_qty,#tot_price,#inv_disc,#inv_bt,#inv_vat,#inv_at,#inv_stamp,#inv_total").attr('disabled', true);
    ...
    function condAdd() {
        ...
        $.post(site_url + 'transaction/accessories/get_number/ARC', function (num) {
    ...
    function read_show2(nav) {
        ...
        $.post(site_url + 'crud/read/?rsl_inv_no=' + rsl_inv_no, {table: table2, nav: nav, id: id}, function (json) {
    ...
    function saveData2() {
        ...
        var save_url2 = site_url + 'transaction/accessories/save_rsld/' + rsl_inv_no + '/<?php echo $_SESSION['C_USER']; ?>';
    ...
    function condDelete2() {
        ...
        $.messager.confirm('Delete Confirmation', 'Do you really want to delete this record?', function (r) {
            if (r) {
                var url = site_url + 'transaction/accessories/delete_rsld';
    ...
    function condReady2() {
        ...
        $('#form_validation2 #part_name,#wrhs_code,#unit,#disc_val,#price_bd,#price_ad, #price_total').attr('disabled', true);
        cmdcondAwal2();
        cmdcondReady2();
