<script>
    ...
    $('#form_validation #dept_name').combogrid({
        onSelect: function (index, row) {
            $('#form_validation #dept_code').val(row.dept_code);
            // $('#form_validation #dept_name').val(row.dept_name);
        }
    });
    $('#form_validation #dunit_name').combogrid({
        onSelect: function (index, row) {
            $('#form_validation #dunit_code').val(row.dunit_code);
    ...
    function read_show(nav) {
        ...
        $.post(site_url + 'transaction/accessories/read_slh/ASA', {table: table, nav: nav, id: id}, function (json) { //alert(json)
            ...
            if (json !== '[]') {
                ...
                $.each(rowData, function (i, v) {
                    ...
                    if(i== 'cust_code'){
                         $("#form_validation #dept_code").val(v);
                    }
                    if(i== 'cust_name'){
                         $("#form_validation #dept_name").combogrid('setValue', v);
                    }
                    if (i == 'wrhs_code' || i == 'dunit_name') {
    ...
    function print_sc(key) {
         ...
         var url = site_url + 'transaction/accessories/outputpdf/<?php echo encrypt_decrypt('encrypt', 'acc_slh');?>/' + ids + '/<?php echo $_SESSION['C_USER']; ?>/'+key+'/<?php echo encrypt_decrypt('encrypt', 'sal_inv_no');?>/<?php echo encrypt_decrypt('encrypt', 'ASA');?>#toolbar=0';
    ...
    function table_grid(ttable, sal_inv_no) {
        ttable.datagrid({
            ...
            columns: [[
                    ...
                    {field: 'part_name', title: '<?php getCaption("Nama Barang"); ?>', width: 270, height: 20, sortable: true},
                    ...
                    {field: 'price_bd', title: '<?php getCaption("Harga Satuan"); ?>', width: 80, height: 20, sortable: true, align: 'right',formatter: formatNumber},
    ...
    function condReady() {
        ...
        $('#form_validation #sal_date, #opn_date, #cls_date').datebox('disable');
        $("#form_validation #sal_inv_no,#inv_type,#dept_code,#dunit_code,#tot_item, #tot_qty,#tot_price,#inv_disc,#inv_bt,#inv_vat,#inv_at,#inv_stamp,#inv_total").attr('disabled', true);
    ...
    function condAdd() {
        ...
        $.post(site_url + 'transaction/accessories/get_number/ASA', function (num) { 
    ...
    function condReady2() {
        ...
        $('#form_validation2 #part_name,#wrhs_code,#unit,#disc_val,#price_ad, #price_total, #price_bd').attr('disabled', true);
        cmdcondAwal2();
        cmdcondReady2();
