<script>
    ...
    $('#cust_name').combogrid({
        onSelect: function (index, row) {
            ...
            if (row.postaddr == 1) {
                ...
                $("#cust_area").val(row.oarea);
                $("#cust_city").val(row.ocity);
                ...
                $("#cust_area").val(row.harea);
                $("#cust_city").val(row.hcity);
            ...
            if (json !== '[]') {
                ...
                */
                if (rowData.cls_date == null || rowData.cls_date == '0000-00-00') {
                    ...
                    $('#cmdOptional').linkbutton('enable');
                    $("#closeOn").empty().append('<button type="button" id="cmdClose" title="<?php getCaption("Close"); ?>" class="easyui-linkbutton easyui-tooltip btn btn-success"   onclick="rolesClose()">Close</button>');
                    ...
                    $('#cmdOptional').linkbutton('disable');
                    $("#closeOn").empty().append('<button type="button" id="cmdUnClose" title="<?php getCaption("unClose"); ?>" class="easyui-linkbutton easyui-tooltip btn btn-success"  onclick="rolesUnclose()">Unclose</button>');
    ...
    function condDelete() {
        ...
        if(pick_date ==''){
            ...
            showAlert("Error while delete", '<font color="red">Maaf, Faktur Jual Kendaraan tidak bisa dihapus</font>');
        }
        ...
        if (cust_type !== undefined) {
            $.messager.confirm('Closing Invoice', 'Close this Invoice?', function (r) {
                if (r) {
                    ...
                    if ($("#due_day").val() == '') {
                        alert('Masukkan Jumlah hari Jatuh Tempo');
                        return false;
                    }

                    if ($("#veh_price").val() == 0) {
                        alert('Silahkan masukkan Harga Kendaraan');
                        return false;
                    }

                    if ($("#salpaytype:checked").val() == 0) {
                        alert('Masukkan Transaksi (Cash atau Credit)');
                    }
            ...
            alert('Jenis Pelanggan harus End User (Pemakai Langsung) atau Dealer/Reseller');
        }
    ...
    function table_uj() {
        ...
        $("#tableUJ").datagrid({
            ...
            pagination: true,
            columns: [[
                    {field: 'dp_inv_no', title: 'No Faktur UJ', width: 120, height: 20, sortable: true},
                    {field: 'dp_date', title: 'Tgl.Faktur UJ', width: 120, height: 20, sortable: true, formatter:formatDate},
                    {field: 'so_no', title: 'No.SPK', width: 100, height: 20, align: 'center', sortable: true},
                    {field: 'payer_name', title: 'Nama Customer', width: 100, height: 20, align: 'center', sortable: true},
                    {field: 'used_val', title: 'Uang Jaminan', width: 100, height: 20, align: 'center', sortable: true},
                    {field: 'check_no', title: 'No. Cek/Giro', width: 100, height: 20, align: 'center', sortable: true},
                    {field: 'chassis', title: 'NO. Chassis', width: 100, height: 20, align: 'center', sortable: true},
                    {field: 'engine', title: 'No. Engine', width: 100, height: 20, align: 'center', sortable: true},
                    {field: 'cust_code', title: 'Kode Customer', width: 100, height: 20, align: 'center', sortable: true}
        ...
        ttable.datagrid({
           ...
           // title: 'Deskripsi Service',
            ...
            columns: [[
                    ...
                    {field: 'wk_desc', title: '<?php getCaption("Nama Pekerjaan"); ?>', width: 370, height: 20, sortable: true},
    ...
    function table_grid_stk(ttable) {
        ttable.datagrid({
            method: 'post',
            url: site_url + 'builder/grid_spk_stok/grid/veh_stk/?grid=true',
            title: 'Silahkan Pilih Dari Stock Kendaraan',
            idField: 'id',
            /*fitColumns: true,*/
            singleSelect: true,
            nowrap: true,
            ...
            pagination: true,
            columns: [[
                    {field: 'chassis', title: 'Chassis', width: 180, height: 20, sortable: true},
                    {field: 'engine', title: 'Engine', width: 180, height: 20, sortable: true},
                    {field: 'stk_date', title: 'Tgl.Stok', width: 100, height: 20, sortable: true, formatter:formatDate},
                    {field: 'match_date', title: 'Tgl. Match', width: 100, height: 20, sortable: true, formatter:formatDate},
                    {field: 'veh_code', title: 'Kode Kendaraan', width: 100, height: 20, sortable: true},
                    {field: 'veh_model', title: 'Model', width: 100, height: 20, sortable: true},
                    {field: 'color_code', title: 'Kode Warna', width: 100, height: 20, sortable: true},
                    {field: 'veh_transm', title: 'Transmisi', width: 100, height: 20, sortable: true},
                    {field: 'veh_name', title: 'Nama Kendaraan', width: 250, height: 20, sortable: true},
                    {field: 'color_name', title: 'Warna Kendaraan', width: 250, height: 20, sortable: true},
                    {field: 'sal_inv_no', title: 'Faktur Beli', width: 150, height: 20, sortable: true},
                    {field: 'so_no', title: 'No.SPK', width: 150, height: 20, sortable: true},
                    {field: 'so_date', title: 'Tgl. SPK', width: 100, height: 20, sortable: true, formatter:formatDate},
    ...
    function cmdPick() { 
        ...
        $.post(site_url + '<?php echo 'transaction/vehicle/check_sld/' . encrypt_decrypt('encrypt', 'veh_sld'); ?>/' + sal_inv_no, function (json) {
            ...
            if (json !== '0') {
                showAlert("Invoice already has optional", '<font color="red">Pekerjaan / Optional sudah ada di Faktur Jual ini. Silahkan dihapus terlebih dahulu.</font>');
            } else {
                $.messager.confirm('Pick Kendaraan', 'Nama pelanggan '+cust_name+' bisa terganti jika SPK sudah Matched. <br />Tetap Pick Kendaraan?', function (r) {
                    if (r) {
                        $("#w").empty().append('<table class="table"><tr><td><table id="tb_stk" class="easyui-datagrid"  title="" style="width:1000px;height:350px;"></table></td></tr>\n\
         `                         <tr><td></td></tr><tr><td> <button type="button" id="GetPick" title="Ok" class="btn btn-success" onclick="get_pick()">Ok</button>&nbsp; &nbsp;<button type="button" title="Cancel" class="btn btn-danger" onclick="$(\'#w\').window(\'close\');">Cancel</button></td></tr>');
                        table_grid_stk($("#tb_stk"));
                        $('#w').window('open');
                    } else {
                        showAlert("Pick Kendaraan Batal", 'Tidak jadi pick kendaraan');
           ...
           if (status !== false) {
                    $.ajax({
                        ...
                        success: function (result) { 
                            ...
                            {
                                showAlert("Error Pick", '<font color="red">' + obj.message + '</font>');
                            }
                        }});
                } else {
                    $('#w').window('close');
                 ...
                 $('#w').window('close');
                showAlert("Customer Information Needed", '<font color="red">Kendaraan ini belum Matching.<br />Masukkan Data Customer lalu Pick</font>');
            }
        }
    ...
    function cmdDropPick() {
        ...
        $.messager.confirm('Drop Kendaraan', 'Drop Kendaraan ini dan kembalikan  ke Stock?', function (r) {
            if (r) {
                //$('#form_validation :input').attr('disabled', false);
                ...
                $.post(url, {table: table, id: $("#id").val(), so_no: $("#so_no").val()}, function (result) {
                    ...
                    {                       
                        showAlert("Error drop", '<font color="red">' + obj.message + '</font>');
                    }
                });
    ...
    function condAdd2() {
        ...
        $('#form_validation2 #disc_pct').keyup(function () {
            ...
            if (parseInt(jml_disc) > parseInt(harga)) {
                alert('Diskon tidak boleh melibihi harga awal!');
                $(this).val('');
            } else {
                var total = harga - jml_disc;
                $('#form_validation2 #disc_val').numberbox('setValue', jml_disc);
    ...
    function print_window(action) {
        ...
        $.post(site_url + 'transaction/vehicle/load_invoice', {table: table, id: id}, function (json) { //alert(json)
           ...
           // var pay_total = rowData.inv_total - (rowData.pay_val + rowData.used_val);
            var pay_total = rowData.pay_val - rowData.used_val;
            var price_ad = rowData.inv_total -  rowData.pay_val;
            $("#form_validation3 #pay_total").numberbox('setValue', pay_total);
            $("#form_validation3 #price_ad").numberbox('setValue', price_ad);
        });
    ...
    function screenSJ() {
        ...
        var url = site_url + 'transaction/vehicle/outputpdf/' + table + '/' + ids + '/<?php echo $session['C_USER']; ?>/screen/' + type;
        window.open(url);
    ...
    function downloadSJ() {
        ...
        var ids = $("#form_validation #id").val();
        var url = site_url + 'transaction/vehicle/outputpdf/' + table + '/' + ids + '/<?php echo $session['C_USER']; ?>/download/' + type;
        $("#screenWindow").empty().append('<iframe frameborder="0"  src="' + url + '" style="width:100%;height:auto;min-height: 500px;padding:0px;"></iframe>');
        read_show('');
    }
    ...
    function print_sc(key) {
        ...
        if (key == 'screen') {
            ...
            if (action == 'faktur') {
                ...
                //$("#screenWindow").window('open');
                var ids = $("#form_validation #id").val();
                var url = site_url + 'transaction/vehicle/outputpdf/' + table + '/' + ids + '/<?php echo $session['C_USER']; ?>/screen';
        ...
        if (key == 'print') {
            var action = $("#form_validation3 #printAction").val();
            var ids = $("#form_validation #id").val();
            var url = site_url + 'transaction/vehicle/outputpdf/' + table + '/' + ids + '/<?php echo $session['C_USER']; ?>/download';
        ...
        $('.radio').change(function () {
            if ($(this).is(':checked')) {
                ...
                if (type == 'debitur') {
                    if (lease_code == '') {
                        $("#form_validation4 input:radio").prop("checked", false);
                        alert('Tidak Ada Nama di Data Debitur/QQ');
