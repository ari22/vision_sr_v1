<script>
        ...
        $.post(site_url + 'crud/read', {table: table, nav: nav, id: id}, function (json) {
            ...
            if (json !== '[]') {
                ...
                $.each(rowData, function (i, v) {
                    ...
                    if (i == 'inv_bt' || i == 'inv_vat' || i == 'inv_stamp' || i == 'inv_total' || i == 'pd_begin' || i == 'pd_paid' || i == 'pd_disc' || i == 'pd_end') {
                        $("#" + i).numberbox('setValue', v);
                    }

                    if (i == 'sal_date' || i == 'opn_date' || i == 'due_date') {
                        if (v !== '0000-00-00') {
               ...
               // $("#inv_vat").numberbox('setValue', -300000);
                $("#payer_name").val(rowData.cust_name);
                $("#payer_addr").val(rowData.cust_addr);
                $("#payer_area").val(rowData.cust_area);
                $("#payer_city").val(rowData.cust_city);
                $("#payer_zipc").val(rowData.cust_zipc);
    ...
    function table_grid(ttable, sal_inv_no) {
        ttable.datagrid({
            method: 'post',
            url: site_url + '<?php echo 'builder/datagrid/' . encrypt_decrypt('encrypt', 'acc_ard'); ?>/sal_inv_no/' + sal_inv_no + '/?grid=true',
            idField: 'id',
            fitColumns: false,
            singleSelect: true,
            nowrap: true,
            ...
            columns: [[
                    ...
                    {field: 'kwit_odate', title: '<?php getCaption("Tgl. Kwitansi"); ?>', width: 120, height: 20, sortable: true, formatter:formatDate},
        ...
        $.post(site_url + 'crud/read/?sal_inv_no=' + sal_inv_no, {table: table2, nav: nav, id: id}, function (json) { // alert(json)
            ...
            if (json !== '[]') {
                ...
                $.each(rowData, function (i, v) {
                    ...
                    if (i == 'pay_type' || i == 'bank_code' || i == 'coll_code') {
    ...
    function saveData2() {
        ...
        var save_url2 = site_url + 'transaction/cashier/save_acc_ard/' + sal_inv_no + '/<?php echo $session['C_USER']; ?>';
    ...
    function condDelete2() {
        ...
        $.messager.confirm('Warning', 'Do you really want to delete this record?', function (r) {
            if (r) {
                ...
                var url = site_url + 'transaction/cashier/deleteAccArd';
    ...
    function table_dpcd(dp_inv_no) {
        /*
         $.post(site_url + '<?php echo 'builder/grid_dpcd/' . encrypt_decrypt('encrypt', 'veh_dpcd'); ?>/' + dp_inv_no + '/piutang/?grid=true', function(json){
         alert(json);
         });
         return false;*/
        ...
        $("#dt_uj").datagrid({
            ...
            columns: [[
                    ...
                    {field: 'due_date', title: '<?php getCaption("Uang Jaminan"); ?>', width: 120, height: 20, sortable: true},
                    {field: 'pay_val', title: '<?php getCaption("Tgl. Terima"); ?>', width: 120, height: 20, sortable: true, formatter:formatDate},
                    {field: 'check_no', title: '<?php getCaption("No. Check"); ?>', width: 120, height: 20, sortable: true},
    ...
    function print_sc(key) {
        ...
        if (key == 'screen') {
            ...
            var url = site_url + 'transaction/cashier/outputpdf/<?php echo encrypt_decrypt('encrypt', 'acc_ard');?>/' + ids + '/<?php echo $session['C_USER']; ?>/screen/' + signature + '/' + jabatan+'#toolbar=0';
            window.open(url);
        ...
        if (key == 'invoice') {
             ...
             if(ids !== ''){
                $.post(site_url + 'transaction/cashier/get_number/AKP', function (num) {
                    $("#no_kwitansi").val(num);
                });
        ...
        if (key == 'print') { 
            ...
            var url = site_url + 'transaction/cashier/outputpdf/<?php echo encrypt_decrypt('encrypt', 'acc_ard');?>/' + ids + '/<?php echo $session['C_USER']; ?>/download/' + signature + '/' + jabatan;
