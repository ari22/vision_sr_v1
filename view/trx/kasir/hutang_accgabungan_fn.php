<script>
        ...
        $.post(site_url + 'crud/read', {table: table, nav: nav, id: id}, function (json) {
            ...
            if (json !== '[]') {
                ...
                /*$('#form_validation').form('load', {
                    ...
                    supp_code: rowData.paid2_code,
                    supp_name: rowData.paid2_name,
                    note: rowData.note,
                ...
                $.each(rowData, function (i, v) {
                    ...
                    if(i == 'wrhs_code' || i == 'bank_code' || i == 'pay_type' || i == 'coll_code'){
                        $("#form_validation #" + i).combogrid('setValue', v);
                    }
                    if(i == 'tot_disc' || i == 'tot_paid'){
                        $("#form_validation #" + i).numberbox('setValue', v);
                    }
                    if(i == 'pinv_code'){
                         $("#form_validation #" + i).combobox('setValue', v);
                    }
                    if (i == 'apg_date' || i == 'opn_date' || i == 'pay_date' || i == 'check_date' || i == 'due_date' || i == 'cls_date') {
                        if (v !== '0000-00-00') {
                ...
                $("#form_validation #supp_name").combogrid('setValue',rowData.paid2_name);
                $("#form_validation #supp_code").val(rowData.paid2_code);
    ...
    function print_sc(key) {
        if (key == 'print') {
            ...
            var url = site_url + 'transaction/cashier/outputpdf/<?php echo encrypt_decrypt('encrypt', 'acc_apgh'); ?>/' + ids + '/<?php echo $session['C_USER']; ?>/print';
        ...
        if (key == 'screen') {
            var ids = $("#form_validation #id").val();
            var url = site_url + 'transaction/cashier/outputpdf/<?php echo encrypt_decrypt('encrypt', 'acc_apgh'); ?>/' + ids + '/<?php echo $session['C_USER']; ?>/screen#toolbar=0';
            window.open(url);
        }
        if (key == 'download') {
            var ids = $("#form_validation #id").val();
            var url = site_url + 'transaction/cashier/outputpdf/<?php echo encrypt_decrypt('encrypt', 'acc_apgh'); ?>/' + ids + '/<?php echo $session['C_USER']; ?>/download';
    ...
    function table_grid(ttable, apg_inv_no) {
        ...
        ttable.datagrid({
            method: 'post',
            url: site_url + '<?php echo 'builder/datagrid/' . encrypt_decrypt('encrypt', 'acc_apgd'); ?>/apg_inv_no/' + apg_inv_no + '/?grid=true',
            idField: 'id',
            fitColumns: false,
            singleSelect: true,
            nowrap: true,
    ...
    function condAdd() {
        ...
        $('#form_validation #pinv_code').combobox('enable');
        ...
        $.post(site_url + 'transaction/cashier/get_number/APG', function (num) {
    ...
    function cmdDetails() {
        ...
        //  var pinv_code = $("#form_validation #pinv_code").combogrid('getValue');   

        tableId2();
        read_show2('');
        table_grid(ttable, apg_inv_no);
        //alert("<?php echo $site_url . 'builder/combogrid/' . encrypt_decrypt('encrypt', 'acc_aph') . '/supp_code/'; ?>" + supp_code + '/pinv_code/'+pinv_code)
        $("#DetailWindow").window('open');
        $('#form_validation2 #pur_inv_no').combogrid({
            panelWidth: 650,
            mode: 'remote',
            idField: 'pur_inv_no',
            textField: 'pur_inv_no',
            fitColumns: false,
            pagination: true,
            sortName: 'pur_inv_no',
            sortOrder: 'asc',
            remoteSort: true,
            multiSort: true,
            url: "<?php echo $site_url . 'builder/combogrid/' . encrypt_decrypt('encrypt', 'acc_aph') . '/supp_code/'; ?>" + supp_code + '/pinv_code/' + pinv_code,
            columns: [[
                    ...
                    {field: 'supp_invno', title: '<?php getCaption("No. Faktur Supplier"); ?>', width: 250},
                    {field: 'supp_invdt', title: '<?php getCaption("Tgl. Faktur Supplier"); ?>', width: 90, formatter: formatDate},
            ...
            onSelect: function (index, rowData) {
                ...
                $.each(rowData, function (i, v) {
                    ...
                    if (i == 'pur_date' || i == 'po_date' | i == 'supp_invdt') {
                        if (v !== '0000-00-00') {
                            var vdate = dateSplit(v);
    ...
    function saveData2() {
        ...
        var save_url2 = site_url + 'transaction/cashier/save_apgd/' + apg_inv_no + '/<?php echo $session['C_USER']; ?>/acc';
    ...
    function condDelete2() {
        ...
        $.messager.confirm('Delete Confirmation', 'Do you really want to delete the selected data?', function (r) {
            if (r) {
                var url = site_url + 'transaction/cashier/delete_apgd/acc';
