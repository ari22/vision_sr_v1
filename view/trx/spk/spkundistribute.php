...
<div style="margin: 10px;" id="form_content">
    <div class="single-form">
        ...
        <!-- <div class="single-form">
             <table id="dg" class="easyui-datagrid"  title="" style="width:820px;height:350px;"
                    data-options="url:'services/runCRUD.php?lookup=trx/spk_undistribute&func=read&where1=1&where2=2',rownumbers:true,singleSelect:false,method:'post',toolbar:toolbar,remoteSort:true,multiSort:true,pagination:true">
                 <thead>
                     <tr>
    ...
    <div class="main-nav">
        <table width="100%">
            <td >
                <a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-no'"  onclick="rolesUnDistSPK();">Un-Distribute</a>
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                <!--<a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-print'" onclick="printPage();">Print</a>-->
            </td>
...
<script type="text/javascript">
    ...
    var urls = site_url + '<?php echo 'builder/grid_spkreg/' . encrypt_decrypt('encrypt', 'veh_spkreg'); ?>/undistribute/?grid=true';

    function rolesUnDistSPK() {
    ...
    function keywordSearch() {
        ...
        var urls = site_url + '<?php echo 'builder/grid_spkreg/' . encrypt_decrypt('encrypt', 'veh_spkreg'); ?>/undistribute/' + viewShow + '/' + keyword + '/?grid=true';
    ...
    function dateSearch() {
        ...
        if (Date.parse(from) / 1000 > Date.parse(to) / 1000) {
            ...
            var urls = site_url + '<?php echo 'builder/grid_spkreg/' . encrypt_decrypt('encrypt', 'veh_spkreg'); ?>/undistribute/' + viewShow + '/' + from + '/' + to + '/?grid=true';
            tableSPKReg(urls);
        }
    ...
    function getSelections() {
        ...
        link = "&lookup=trx/spk_undistribute&func=undist&list_so_no=" + data;
        $.post(url, link)
                .done(function (data) {
                    obj = JSON.parse(data);
                    if (obj.success == true) {
    ...
    function printPage() {
        var tableData = $("#dg").html();
        alert(tableData);
        return false;
        //var tableData = '<table border="1">' + document.getElementsById('dg').innerHTML + '</table>';
