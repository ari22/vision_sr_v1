    ...
    <table width="100%">
        <tr>
            <td valign="top">
                ...
                <div id="hasilTableDiv" style="display:none">
                    <table id="tableSearch" class="easyui-datagrid" style="max-height: 200px;height:200px;"></table>
                </div>
        ...
        <tr>
            <td width="350" valign="top">
                ...
                <button type="button" class="easyui-linkbutton easyui-tooltip btn btn-default" data-options="iconCls:'icon-redo'" onclick="closewindow()">Quit</button>
                <hr />
                    ...
                    <table id="tempat" class="table table-condensed" style="margin-left: -5px;">
                        <tbody id="body1">
                            ...
                            <tr id='Divno0' class='row'>
                                ...
                                <td><select class='combo comboSearch'  id='SPK0' name='data[]' onchange="tanggal('0')" height='30px' style='width: 145px'>
                                </select></td>
                                <td><select  class='combo comboSearch'  id='sel_type0' name='data[]' onchange="showTextBox('0')" height='30px' style='width: 90px'>
...
<script>
    ...
    function add() {
        ...
        var tabel = "<tr id='" + divIdName + "' class='row'>" +
                ...
                "<td><select class='combo comboSearch'  id='SPK" + hasil + "' name='data[]' height='30px' style='width: 145px'>" +
                "</select></td>" +
                "<td><select  class='combo comboSearch'  id='sel_type" + hasil + "' name='data[]' onchange=\"showTextBox('" + hasil + "')\" height='30px' style='width: 90px'>" +
                "<option value='1'>ALL          </option>" +
                "<option value='2'>=		</option>" +
                "<option value='3'>between	</option>" +
                "<option value='4'>start with   </option>" +
    ...
    function clickOK(){
            ...
            if(table_used == 'acc_slh'){
                  var cond = '<?php echo $_POST['cond']; ?>';
                  var search = search_url + 'search_function.php?table=' + table_used + '&'+ str+'&'+'sinv_code='+cond;
            }else{
            ...
            $('#tableSearch').datagrid({
                ...
                columns: [[
                        <?php
                        $array = array('pay_val','disc_val','inv_total','pd_begin','pd_paid','pd_disc','pd_end','hd_begin','hd_paid','hd_disc','hd_end','tot_paid','veh_price','dp_end', 'total', 'qty','qty_order','qty_border','qty_pick','qty_border');
                            foreach ($arr as $key => $isi) {
                                if (strpos($isi, 'date') !== false) {
                                    echo "{field: '$isi', title: '$key', formatter:formatDate, sortable: true},";
                                }
                                elseif (strpos($isi, 'invdt') !== false) {
                                    echo "{field: '$isi', title: '$key', formatter:formatDate, sortable: true},";
                                }
