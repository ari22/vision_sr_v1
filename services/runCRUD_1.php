...
function outputReport($output = null, $tbl = null, $filename = null, $statsql = null, $tblimport = null) {
    $out = '';
    if (isset($output)) {
        switch ($output) {
            ...
            case 'export' :
                if($statsql !== null){
                    ...
                    header('Expires: 0');
                    $tblexport = exportCsv($statsql, $tblimport);
...
function exportCsv($statsql, $tblimport) {
    error_reporting(E_ALL ^ E_NOTICE);
    error_reporting(0);

    include 'export_label.php';

    $tables = tblrows($tblimport);
