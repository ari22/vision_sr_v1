...
if (isset($func))
{
	...
	{		
		if (/*isset($field1) && */isset($field2))
		{
			...
			}else
			{
				...
				$sql="update $table set $field1='".strtoupper($$field1)."', $field2='".strtoupper($$field2)."', add_by='".$_SESSION['C_USER']."', add_date='".date('Y-m-d')."' where $field1='".$$field1."'";
				$result=mysql_query($sql);
	...
	if ($func=='update')
	{
		...
		if (isset($field1) && isset($field2))
		{
			...
			{					
				//mau dicoba versi sendiri
				$query_update="update $table set $field2='".strtoupper($$field2)."',pur_price='$pur_price',sal_price='$sal_price' where id=$id";
