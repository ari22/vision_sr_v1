	...
	{		
		if (/*isset($field1) && */isset($field2))
		{
			...
			}else
			{
				...
				$query = "select id from $table where srep_name = '$srep_name'";
				//echo $query;
				$result = mysql_query($query);
				$row = mysql_fetch_assoc($result);
				$id = $row['id'];
				//update pk dan sk jadi kapital
				$sql="update $table set $field1='".strtoupper($$field1)."', $field2='".strtoupper($$field2)."' where id=$id";
				$result=mysql_query($sql);
	...
	if ($func=='update')
	{
		...
		if (isset($field1) && isset($field2))
		{
			...
			}else
			{
				...
				$query_update="update $table set $field2='".strtoupper($$field2)."', sex='$sex',srep_lev='$srep_lev',in_date='$in_date',out_date='$out_date',dob='$dob',pob='$pob',marital_st='$marital_st',haddr='$haddr',hphone='$hphone',hcity='$hcity',hzipcode='$hzipcode',hp='$hp',highestedu='$highestedu',train_exp='$train_exp',relig_code='$relig_code',note='$note' where id=$id";
